...
from .agent import YahooFinanceAgent

logger = logging.getLogger(__name__)

class YahooFinanceServer:
    """Server for Yahoo Finance data with MCP integration."""
    
    def __init__(self, server_name: str = "Investor-Agent"):
        """
        Initialize the Yahoo Finance server.
    ...
    def _register_tools(self) -> None:
        ...
        self.mcp.tool()(self.get_ticker_data)
        self.mcp.tool()(self.get_available_options)
        self.mcp.tool()(self.get_price_history)
        self.mcp.tool()(self.get_financial_statements)
        self.mcp.tool()(self.get_institutional_holders)
        self.mcp.tool()(self.get_earnings_history)
        self.mcp.tool()(self.get_insider_trades)
    
    def get_ticker_data(self, ticker: str) -> str:
        ...
        try:
            info = self.agent.get_ticker_info(ticker)
            if not info:
            ...
            # Calendar dates
            if calendar := self.agent.get_calendar(ticker):
                dates_data = []
            ...
            if (recommendations := self.agent.get_recommendations(ticker)) is not None and not recommendations.empty:
                rec_data = [
                    [
            ...
            if (upgrades := self.agent.get_upgrades_downgrades(ticker)) is not None and not upgrades.empty:
                upg_data = [
                    [
    ...
    def get_available_options(
        ...
        try:
            df, error = self.agent.get_filtered_options(
    ...
    def get_price_history(
        ...
        history = self.agent.get_price_history(ticker, period)
    ...
    def get_financial_statements(
        ...
        data = self.agent.get_financial_statements(ticker, statement_type, frequency)
    ...
    def get_institutional_holders(self, ticker: str, top_n: int = 20) -> str:
        """Get major institutional and mutual fund holders."""
        inst_holders, fund_holders = self.agent.get_institutional_holders(ticker, top_n)
        ...
        def format_holder_data(df: pd.DataFrame) -> List[List[Any]]:
            return [
                [
                    row['Holder'],
                    f"{row['Shares']:,.0f}",
    ...
    def get_earnings_history(self, ticker: str) -> str:
        ...
        earnings_history = self.agent.get_earnings_history(ticker)
    ...
    def get_insider_trades(self, ticker: str) -> str:
        """Get recent insider trading activity."""
        trades = self.agent.get_insider_trades(ticker)
    ...
    def run(self) -> None:
        ...
        # This method would be used to start the server
        # If MCP has its own start method
        """Start the MCP server."""
        try:
            print("Running MCP Server for Investor Analysis...", file=sys.stderr)
